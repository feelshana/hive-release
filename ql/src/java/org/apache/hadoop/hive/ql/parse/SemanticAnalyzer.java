...
public class SemanticAnalyzer extends BaseSemanticAnalyzer {
    ...
    public SemanticAnalyzer(HiveConf conf) throws SemanticException {
        ...
        autogenColAliasPrfxLbl = HiveConf.getVar(conf,
                ConfVars.HIVE_AUTOGEN_COLUMNALIAS_PREFIX_LABEL);
    ...
    private void assertCombineInputFormat(Tree numerator, String message) throws SemanticException {
        String inputFormat = conf.getVar(ConfVars.HIVE_EXECUTION_ENGINE).equals("tez") ?
                HiveConf.getVar(conf, ConfVars.HIVETEZINPUTFORMAT) :
                HiveConf.getVar(conf, ConfVars.HIVEINPUTFORMAT);
        ...
        if (ast.getToken() != null) {
            ...
            switch (ast.getToken().getType()) {
                ...
                case HiveParser.TOK_ANALYZE:
                    ...
                    HiveConf.setVar(conf, ConfVars.HIVEMAPREDMODE, "nonstrict");
            ...
            for (String name : qbp.getClauseNamesForDest()) {
                ...
                switch (ast.getToken().getType()) {
                    case HiveParser.TOK_TAB: {
                        ...
                        if (HiveConf.getBoolVar(conf, ConfVars.HIVESTATSAUTOGATHER)) {
...
//    if (hdfsEncryptionShim != null) {
    ...
    private void replaceViewReferenceWithDefinition(QB qb, Table tab,
        ...
        try {
            ...
            nodeOriginTagger.startWalking(Collections
        ...
        if ((!this.skipAuthorization() && !qb.isInsideView() && HiveConf.getBoolVar(conf,
                ...
                || HiveConf.getBoolVar(conf, ConfVars.HIVE_STATS_COLLECT_SCANCOLS)) {
        ...
        if (leftCondAl1.size() != 0) {
            if ((rightCondAl1.size() != 0)
                ...
                if (type.equals(JoinType.LEFTOUTER) ||
                    ...
                    if (conf.getBoolVar(ConfVars.HIVEOUTERJOINSUPPORTSFILTERS)) {
    ...
    private Operator genScriptPlan(ASTNode trfm, QB qb, Operator input)
        ...
        TableDesc errInfo;
        TableDesc inInfo;
        String defaultSerdeName = conf.getVar(ConfVars.HIVESCRIPTSERDE);
        ...
        if (HiveConf.getBoolVar(conf, ConfVars.HIVESCRIPTESCAPE)) {
    ...
    private Class<? extends RecordReader> getRecordReader(ASTNode node)
        ...
        if (node.getChildCount() == 0) {
            name = conf.getVar(ConfVars.HIVESCRIPTRECORDREADER);
        } else {
    ...
    private Class<? extends RecordWriter> getRecordWriter(ASTNode node)
        ...
        if (node.getChildCount() == 0) {
            name = conf.getVar(ConfVars.HIVESCRIPTRECORDWRITER);
        } else {
            name = unescapeSQLString(node.getChild(0).getText());
        }
    ...
    static boolean isRegex(String pattern, HiveConf conf) {
        String qIdSupport = HiveConf.getVar(conf, ConfVars.HIVE_QUOTEDID_SUPPORT);
     ...
     *
     * @see #handleInsertStatementSpecPhase1(ASTNode, QBParseInfo, Phase1Ctx)
     * @throws SemanticException
     */
    ...
    public static GenericUDAFInfo getGenericUDAFInfo(GenericUDAFEvaluator evaluator,
                                                     Mode emode, ArrayList<ExprNodeDesc> aggParameters)
            throws SemanticException {
    ...
    public static Mode groupByDescModeToUDAFMode(
            GroupByDesc.Mode mode, boolean isDistinct) {
        switch (mode) {
            case COMPLETE:
                return Mode.COMPLETE;
            case PARTIAL1:
                return Mode.PARTIAL1;
            case PARTIAL2:
                return Mode.PARTIAL2;
            case PARTIALS:
                return isDistinct ? Mode.PARTIAL1
                        : Mode.PARTIAL2;
            case FINAL:
                return Mode.FINAL;
            case HASH:
                return Mode.PARTIAL1;
            case MERGEPARTIAL:
                return isDistinct ? Mode.COMPLETE
                        : Mode.FINAL;
            default:
    ...
    private Operator genGroupByPlanGroupByOperator(QBParseInfo parseInfo,
        ...
        float groupByMemoryUsage = HiveConf.getFloatVar(conf, ConfVars.HIVEMAPAGGRHASHMEMORY);
        float memoryThreshold = HiveConf
                .getFloatVar(conf, ConfVars.HIVEMAPAGGRMEMORYTHRESHOLD);
    ...
    private ReduceSinkOperator genCommonGroupByPlanReduceSinkOperator(QB qb, List<String> dests,
        ...
        ReduceSinkDesc rsDesc = PlanUtils.getReduceSinkDesc(reduceKeys, keyLength, reduceValues,
                ...
                true, -1, keyLength, numReducers, Operation.NOT_ACID);
    ...
    private Operator genGroupByPlanMapAggrNoSkew(String dest, QB qb,
        ...
        int newMRJobGroupingSetsThreshold =
                conf.getIntVar(ConfVars.HIVE_NEW_JOB_GROUPING_SET_CARDINALITY);
    ...
    private Operator genBucketingSortingDest(String dest, Operator input, QB qb,
        ...
        if ((dest_tab.getNumBuckets() > 0) &&
                (conf.getBoolVar(ConfVars.HIVEENFORCEBUCKETING))) {
        ...
        if ((dest_tab.getSortCols() != null) &&
                ...
                (conf.getBoolVar(ConfVars.HIVEENFORCESORTING))) {
            enforceSorting = true;
        ...
        if (enforceBucketing || enforceSorting) {
            int maxReducers = conf.getIntVar(ConfVars.MAXREDUCERS);
            if (conf.getIntVar(ConfVars.HADOOPNUMREDUCERS) > 0) {
                maxReducers = conf.getIntVar(ConfVars.HADOOPNUMREDUCERS);
            }
            ...
            input = genReduceSinkPlan(input, partnCols, sortCols, order.toString(), maxReducers,
                    (AcidUtils.isAcidTable(dest_tab) ? getAcidType(dest) : Operation.NOT_ACID));
    ...
    protected Operator genFileSinkPlan(String dest, QB qb, Operator input)
        ...
        switch (dest_type.intValue()) {
            case QBMetaData.DEST_TABLE: {
                ...
                if ((!conf.getBoolVar(ConfVars.HIVE_INSERT_INTO_EXTERNAL_TABLES)) &&
                ...
                if (parts != null && parts.size() > 0) { // table is partitioned
                    ...
                    if (dpCtx == null) {
                        ...
                        dpCtx = new DynamicPartitionCtx(dest_tab, partSpec,
                                conf.getVar(ConfVars.DEFAULTPARTITIONNAME),
                                conf.getIntVar(ConfVars.DYNAMICPARTITIONMAXPARTSPERNODE));
                        qbm.setDPCtx(dest, dpCtx);
                    }

                    if (!HiveConf.getBoolVar(conf, ConfVars.DYNAMICPARTITIONING)) { // allow DP
                        throw new SemanticException(generateErrorMessage(
                                qb.getParseInfo().getDestForClause(dest),
                ...
                if (!isNonNativeTable) {
                    Operation acidOp = Operation.NOT_ACID;
                    if (destTableIsAcid) {
            ...
            case QBMetaData.DEST_DFS_FILE: {
                ...
                if (tblDesc == null) {
                    if (qb.getIsQuery()) {
                        String fileFormat = HiveConf.getVar(conf, ConfVars.HIVEQUERYRESULTFILEFORMAT);
        ...
        if (destTableIsAcid) {
            Operation wt = updating(dest) ? Operation.UPDATE :
                    (deleting(dest) ? Operation.DELETE : Operation.INSERT);
        ...
        fileSinkDesc.setStatsCollectRawDataSize(HiveConf.getBoolVar(conf,
                ConfVars.HIVE_STATS_COLLECT_RAWDATASIZE));
    ...
    Operator genConversionSelectOperator(String dest, QB qb, Operator input,
        ...
        boolean dynPart = HiveConf.getBoolVar(conf, ConfVars.DYNAMICPARTITIONING);
...
//todo: this looks different than current 'master'!!!!!!!!
    ...
    private Operator genReduceSinkPlan(String dest, QB qb, Operator<?> input,
        ...
        if (sortExprs == null) {
            ...
            if (sortExprs != null) {
                ...
                if (conf.getVar(ConfVars.HIVEMAPREDMODE).equalsIgnoreCase(
                        "strict")
                        && limit == null) {
    ...
    private Operator genReduceSinkPlan(Operator<?> input,
                                       ...
                                       String sortOrder, int numReducers, Operation acidOp) throws SemanticException {
    ...
    private Operator genJoinReduceSinkChild(QB qb, ExprNodeDesc[] joinKeys,
        ...
        ReduceSinkDesc rsDesc = PlanUtils.getReduceSinkDesc(reduceKeys,
                ...
                reduceKeys.size(), numReds, Operation.NOT_ACID);
    ...
    private List<String> getMapSideJoinTables(QB qb) {
        ...
        for (int pos = 0; pos < hints.getChildCount(); pos++) {
            ...
            if (((ASTNode) hint.getChild(0)).getToken().getType() == HiveParser.TOK_MAPJOIN) {
                ...
                if (!conf.getBoolVar(ConfVars.HIVEIGNOREMAPJOINHINT)
                        && !conf.getVar(ConfVars.HIVE_EXECUTION_ENGINE).equals("tez")) {
    ...
    private QBJoinTree genJoinTree(QB qb, ASTNode joinParseTree,
        ...
        if (qb.getParseInfo().getHints() != null) {
            ...
            if ((conf.getVar(ConfVars.HIVE_EXECUTION_ENGINE).equals("tez")) == false) {
                parseStreamTables(joinTree, qb);
            }
        }
    ...
    private Operator genBodyPlan(QB qb, Operator input, Map<String, Operator> aliasToOpInfo)
        ...
        if (conf.getBoolVar(ConfVars.HIVEMULTIGROUPBYSINGLEREDUCER)) {
            try {
        ...
        if (!commonGroupByDestGroups.isEmpty()) {
            ...
            for (List<String> commonGroupByDestGroup : commonGroupByDestGroups) {
                ...
                if (commonGroupByDestGroup.size() == 1 ||
                        ...
                        conf.getBoolVar(ConfVars.HIVEGROUPBYSKEW) ||
                        !conf.getBoolVar(ConfVars.HIVEMULTIGROUPBYSINGLEREDUCER)) {
                    ...
                    for (String dest : commonGroupByDestGroup) {
                        ...
                        if ((qbp.getAggregationExprsForClause(dest).size() != 0
                            ...
                            if (conf.getBoolVar(ConfVars.HIVEGROUPBYSKEW) &&
                            ...
                            if (conf.getBoolVar(ConfVars.HIVEMAPSIDEAGGREGATE)) {
                                if (!conf.getBoolVar(ConfVars.HIVEGROUPBYSKEW)) {
                                    curr = genGroupByPlanMapAggrNoSkew(dest, qb, curr);
                                } else {
                                    curr = genGroupByPlanMapAggr2MR(dest, qb, curr);
                                }
                            } else if (conf.getBoolVar(ConfVars.HIVEGROUPBYSKEW)) {
    ...
    private Operator genUnionPlan(String unionalias, String leftalias,
        ...
        boolean isMR = HiveConf.getVar(conf,
                ConfVars.HIVE_EXECUTION_ENGINE).equals("mr");
    ...
    private void setupStats(TableScanDesc tsDesc, QBParseInfo qbp, Table tab, String alias,
        ...
        if (!qbp.isAnalyzeCommand()) {
            ...
            tsDesc.setStatsReliable(conf.getBoolVar(ConfVars.HIVE_STATS_RELIABLE));
    ...
    void analyzeInternal(ASTNode ast, PlannerContext plannerCtx) throws SemanticException {
        ...
        if (createVwDesc != null) {
            ...
            resultSchema = convertRowSchemaToResultSetSchema(opParseCtx.get(sinkOp).getRowResolver(),
                    HiveConf.getBoolVar(conf, ConfVars.HIVE_RESULTSET_USE_UNIQUE_COLUMN_NAMES));
        }
        ...
        if (HiveConf.getBoolVar(this.conf, ConfVars.HIVE_STATS_COLLECT_TABLEKEYS) == true) {
        ...
        boolean isColumnInfoNeedForAuth = SessionState.get().isAuthorizationModeV2()
                && HiveConf.getBoolVar(conf, ConfVars.HIVE_AUTHORIZATION_ENABLED);
        if (isColumnInfoNeedForAuth
                || HiveConf.getBoolVar(this.conf, ConfVars.HIVE_STATS_COLLECT_SCANCOLS) == true) {
    ...
    private void enforceScanLimits(ParseContext pCtx, FetchTask fTask)
            throws SemanticException {
        int scanLimit = HiveConf.getIntVar(conf, ConfVars.HIVELIMITTABLESCANPARTITION);
    ...
    public Map<ASTNode, ExprNodeDesc> genAllExprNodeDesc(ASTNode expr, RowResolver input,
        ...
        if (fieldDescList.size() != 0) {
            ...
            Collections.sort(fieldDescList, new Comparator<Entry<ASTNode, ExprNodeDesc>>() {
            ...
            for (Entry<ASTNode, ExprNodeDesc> entry : fieldDescList) {
                ASTNode node = entry.getKey();
    ...
    public void validate() throws SemanticException {
        ...
        boolean reworkMapredWork = HiveConf.getBoolVar(this.conf,
                ConfVars.HIVE_REWORK_MAPREDWORK);
    ...
    private void processPositionAlias(ASTNode ast) throws SemanticException {
        ...
        if (HiveConf.getBoolVar(conf,
                ConfVars.HIVE_GROUPBY_ORDERBY_POSITION_ALIAS) == true) {
            isByPos = true;
        }
...
//--------------------------- Windowing handling: PTFInvocationSpec to PTFDesc --------------------
    ...
    private Operation getAcidType(String destination) {
        return deleting(destination) ? Operation.DELETE :
                (updating(destination) ? Operation.UPDATE :
                        Operation.INSERT);
    }

    private Operation getAcidType(Class<? extends OutputFormat> of, String dest) {
        if (SessionState.get() == null || !SessionState.get().getTxnMgr().supportsAcid()) {
            return Operation.NOT_ACID;
