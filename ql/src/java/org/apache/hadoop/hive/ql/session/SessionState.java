...
//  private HadoopShims.HdfsEncryptionShim hdfsEncryptionShim;
    ...
    public boolean getIsSilent() {
        if (conf != null) {
            return conf.getBoolVar(ConfVars.HIVESESSIONSILENT);
        } else {
            return isSilent;
        }
    }
    ...
    public void setIsSilent(boolean isSilent) {
        if (conf != null) {
            conf.setBoolVar(ConfVars.HIVESESSIONSILENT, isSilent);
        }
    ...
    public SessionState(HiveConf conf, String userName) {
        ...
        isSilent = conf.getBoolVar(ConfVars.HIVESESSIONSILENT);
        ls = new LineageState();
        ...
        if (StringUtils.isEmpty(conf.getVar(ConfVars.HIVESESSIONID))) {
            conf.setVar(ConfVars.HIVESESSIONID, makeSessionId());
        }
        parentLoader = JavaUtils.getClassLoader();
    }

    public void setCmd(String cmdString) {
        conf.setVar(ConfVars.HIVEQUERYSTRING, cmdString);
    }

    public String getCmd() {
        return (conf.getVar(ConfVars.HIVEQUERYSTRING));
    }
    ...
    public String getSessionId() {
        return (conf.getVar(ConfVars.HIVESESSIONID));
    }
    ...
    public synchronized HiveTxnManager initTxnMgr(HiveConf conf) throws LockException {
        ...
        if (txnMgr != null &&
                !txnMgr.getTxnManagerName().equals(conf.getVar(ConfVars.HIVE_TXN_MANAGER))) {
...
// SessionState is not available in runtime and Hive.get().getConf() is not safe to call
        ...
        synchronized public static SessionState start (SessionState startSs){
            ...
            if (startSs.hiveHist == null) {
                if (startSs.getConf().getBoolVar(ConfVars.HIVE_SESSION_HISTORY_ENABLED)) {
            ...
            if (HiveConf.getVar(startSs.getConf(), ConfVars.HIVE_EXECUTION_ENGINE)
        ...
        private void createSessionDirs (String userName) throws IOException {
            ...
            String scratchDirPermission = HiveConf.getVar(conf, ConfVars.SCRATCHDIRPERMISSION);
            Path path;
            // 1. HDFS scratch dir
            ...
            // 2. Local scratch dir
            path = new Path(HiveConf.getVar(conf, ConfVars.LOCALSCRATCHDIR));
            createPath(conf, path, scratchDirPermission, true, false);
            // 3. Download resources dir
            path = new Path(HiveConf.getVar(conf, ConfVars.DOWNLOADED_RESOURCES_DIR));
            createPath(conf, path, scratchDirPermission, true, false);
            ...
            // 5. hold a lock file in HDFS session dir to indicate the it is in use
            if (conf.getBoolVar(ConfVars.HIVE_SCRATCH_DIR_LOCK)) {
            ...
            // 6. Local session path
            localSessionPath = new Path(HiveConf.getVar(conf, ConfVars.LOCALSCRATCHDIR), sessionId);
        ...
        private Path createRootHDFSDir (HiveConf conf) throws IOException {
            Path rootHDFSDirPath = new Path(HiveConf.getVar(conf, ConfVars.SCRATCHDIR));
        ...
        private void setupAuth () {
            ...
            try {
                authenticator = HiveUtils.getAuthenticator(conf,
                        ConfVars.HIVE_AUTHENTICATOR_MANAGER);
                authenticator.setSessionState(this);

                String clsStr = HiveConf.getVar(conf, ConfVars.HIVE_AUTHORIZATION_MANAGER);
        ...
        private static File createTempFile (HiveConf conf) throws IOException {
            String lScratchDir = HiveConf.getVar(conf, ConfVars.LOCALSCRATCHDIR);
            String sessionID = conf.getVar(ConfVars.HIVESESSIONID);
 ...
 */
        ...
        private String downloadResource (String value,boolean convertToUnix){
            if (canDownloadResource(value)) {
                ...
                File resourceDir = new File(getConf().getVar(ConfVars.DOWNLOADED_RESOURCES_DIR));
        ...
        public void close ()throws IOException {
            ...
            File resourceDir =
                    new File(getConf().getVar(ConfVars.DOWNLOADED_RESOURCES_DIR));
        ...
        public void setupQueryCurrentTimestamp () {
            ...
            if (conf.getBoolVar(ConfVars.HIVE_IN_TEST)) {
                String overrideTimestampString =
                        HiveConf.getVar(conf, ConfVars.HIVETESTCURRENTTIMESTAMP, null);
